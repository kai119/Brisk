...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object was not Function. got=%T (%+v)", evaluated, evaluated)
	}
	...
	if fn.Body.String() != expectedBody {
		t.Fatalf("body was not %q. got=%q", expectedBody, fn.Body.String())
	}
}
...
func TestDictionaryLiterals(t *testing.T) {
	input := `var two = "two";
	{
		"one": 10 - 9,
		two: 1 + 1,
	...
	evaluated := testEval(input)
	result, ok := evaluated.(*object.Dictionary)
	if !ok {
		t.Fatalf("Eval didn't return Dictionary. got=%T (%+v)", evaluated, evaluated)
	}

	expected := map[object.DictionaryKey]int64{
		(&object.String{Value: "one"}).DictionaryKey():   1,
		(&object.String{Value: "two"}).DictionaryKey():   2,
		(&object.String{Value: "three"}).DictionaryKey(): 3,
		(&object.Integer{Value: 4}).DictionaryKey():      4,
		TRUE.DictionaryKey():                             5,
		FALSE.DictionaryKey():                            6,
	}

	if len(result.Pairs) != len(expected) {
		t.Fatalf("Dictionary has wrong number of pairs. got=%d", len(result.Pairs))
	}
...
func TestDictionaryIndexExpressions(t *testing.T) {
	tests := []struct {
		input    string
		expected interface{}
	}{
...
func testBooleanObject(t *testing.T, obj object.Object, expected bool) bool {
	...
	if !ok {
		t.Errorf("object is not an boolean. got=%T", obj)
		return false
	}
	if result.Value != expected {
